...
{
    s32 ret = -1;
#if CKT_GTP_DRIVER_SEND_CFG //add by sen.luo
u8 temp_data = 0;
#endif
#if GTP_DRIVER_SEND_CFG
    s32 i;
	...
	#if CKT_GTP_DRIVER_SEND_CFG //add by sen.luo
	vendor_id =  rd_cfg_buf[GTP_ADDR_LENGTH];
	if(0==vendor_id)
		{
ret = i2c_read_bytes(i2c_client_point, GTP_REG_CONFIG_DATA, &temp_data, 1);
	if (ret < 0)
...
GTP_INFO("sen.luo IC CFG version: 0x%x;",temp_data);	

    if(temp_data==(u8)0x46)//bm
	{
	rd_cfg_buf[GTP_ADDR_LENGTH] = 1;
	}
    else
	{
	rd_cfg_buf[GTP_ADDR_LENGTH] = 0;//cs
	}
}
	else if(1==vendor_id)
		{
      rd_cfg_buf[GTP_ADDR_LENGTH] = 2;
		}
	else
		{
	rd_cfg_buf[GTP_ADDR_LENGTH] = 0;
     ...
     //cfg_version = temp_data;//rd_cfg_buf[GTP_ADDR_LENGTH];
...
static struct driver_attribute *gt968_attr_list[] = {
    &driver_attr_chipinfo,
};
static int gt968_create_attr(struct device_driver *driver) 
{
	int idx, err = 0;
	int num = (int)(sizeof(gt968_attr_list)/sizeof(gt968_attr_list[0]));
	if (driver == NULL)
	{
		return -EINVAL;
	}
	...
	{
		if((err = driver_create_file(driver, gt968_attr_list[idx])))
		{            
			printk("driver_create_file (%s) = %d\n", gt968_attr_list[idx]->attr.name, err);
			break;
		}
	}    
	return err;
}
static int gt968_delete_attr(struct device_driver *driver)
{
	int idx ,err = 0;
	int num = (int)(sizeof(gt968_attr_list)/sizeof(gt968_attr_list[0]));
	if (!driver)
	...
	for (idx = 0; idx < num; idx++) 
	{
		driver_remove_file(driver, gt968_attr_list[idx]);
	}
	return err;
}
//add by sen.luo for chipinfo 2013.11.19 end
#endif
//extern struct platform_driver ektf2k_tp_driver;
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    tpd_load_status = 1;
#if 0
	if(err = gt968_create_attr(&ektf2k_tp_driver.driver))
	{
		TPD_DMESG(TPD_DEVICE,"gt968_create_attr ret = %d\n", err);
	}
//add by sen.luo for chipinfo 2013.09.24 end
#endif
GTP_INFO("tpd_i2c_probe ~~successful~~");
    return 0;
out:
    return -1;
}
...
static int tpd_i2c_remove(struct i2c_client *client)
{
 ...
 //add by sen.luo for chipinfo 2013.09.24 start	
   if(err = gt968_delete_attr(&ektf2k_tp_driver.driver))
	{
		TPD_DEBUG("gt968_delete_attr fail: %d\n", err);
	} 
   //add by sen.luo for chipinfo 2013.09.24 endDelay
   #endif
    return 0;
}
#ifdef GTP_CHARGER_DETECT
...
static int touch_event_handler(void *unused)
{
	...
	else
	{
	    ...
	    {
// ËÕ ÓÂ 2013Äê12ÔÂ16ÈÕ 10:36:25		tpd_up( 0, 0, 0);
			for (i = 0; i < TPD_KEY_COUNT; i++)
            {
                //input_report_key(tpd->dev, touch_key_array[i], key_value & (0x01 << i));
